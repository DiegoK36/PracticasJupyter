...
def sec_mult(A, B, n_fil_A, n_col_A, n_fil_B, n_col_B): # f() que calcula la mult. en secuencial, como toda la vida se ha hecho 
...
if __name__ == '__main__':
    ...
    inicioS = time.time()
    X = sec_mult(A, B) # Ejecuto multiplicacion secuencial
    finS = time.time()
    
    inicioP = time.time()
    Y = par_mult(A, B) # Ejecuto multiplicacion paralela
    finP = time.time()
    
    print('\n\nMatriz  A y B se han multiplicado con exito en SECUENCIAL ha tardado ', finS-inicioS, ' y en PARALELO ', finP-inicioP)
    print('\n\nResultado en secuencial', X)
    print('\n\nResultado en secuencial', Y)